...
def calculate_radius_of_curvature(poly_coeffs, x):
    """Calculate the radius of curvature at a specific x."""
    a, b, _ = poly_coeffs
    dydx = 2 * a * x + b  # First derivative
    d2ydx2 = 2 * a        # Second derivative
    R = ((1 + dydx**2)**1.5) / abs(d2ydx2)
    return R

def draw_tangents_and_curvature(image, curve_points):
    """Fit a curve to the road, draw tangents, and calculate/display curvature."""
    ...
    # Compute tangents and radius of curvature at key points
    tangent_points = [x[0], x[len(x) // 2], x[-1]]
    for xi in tangent_points:
        yi = poly(xi)
        dydx = np.polyder(poly)(xi)  # First derivative gives the tangent slope
        R = calculate_radius_of_curvature(poly_coeffs, xi)  # Calculate radius of curvature
        ...
        cv2.putText(image, f"R={R:.2f}", (int(xi), int(yi) - 10),
...
for contour in contours:
    if len(contour) > 10:  # Ignore small contours
        ...
        draw_tangents_and_curvature(resized, curve_points)
...
output_image_path = os.path.join(output_dir, f"curves_tangents_curvature_{118015}.png")
output_edges_path = os.path.join(output_dir, f"edges_{118015}.png")

cv2.imwrite(output_image_path, resized)  # Save the image with curves, tangents, and curvature
cv2.imwrite(output_edges_path, edges)  # Save the edge-detected image

# Step 9: Display the results
cv2.imshow("Curves, Tangents, and Curvature", resized)
